function dispMap = stereoNCCRight(imgL, imgR, winSize, dispRange)
...
for i = 1+hwin+dispRange(2):borderRow                % traverse rows
    for j = 1+hwin+dispRange(2):borderCol            % traverse columns
        ...
        normCoefSegR = sum( sum( imgR(i-hwin:i+hwin, j-hwin:j+hwin).^2 ) );
        for curDisp = dispRange(1):dispRange(2)
            % correlation of the right image segment with the left image
            % segment which is shifted by the current disparity value
            corr = sum( sum( imgR(i-hwin:i+hwin, j-hwin:j+hwin) .* ...
                    imgL(i-hwin:i+hwin, j-hwin+curDisp:j+hwin+curDisp) ) );
            % normalization coefficient
            normCoefSegL = sum( sum( imgL(i-hwin:i+hwin, ...
